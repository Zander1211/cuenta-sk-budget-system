...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontWeight: 700, fontSize: '14px', color: '#0f172a', marginBottom: '10px' }}>
      ...
      {payload.map((entry) => (
        ...
        >
          ...
          <span style={{ fontWeight: 600, color: '#0f172a' }}>{peso.format(entry.value)}</span>
        </div>
      ))}
    </div>
  )
    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#64748b', fontWeight: 500 }}>
            <span style={{ display: 'inline-block', width: 28, height: 3, borderRadius: 2, background: '#6366f1' }} />
            Allocated Budget
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#64748b', fontWeight: 500 }}>
            <span style={{ display: 'inline-block', width: 28, height: 3, borderRadius: 2, background: '#f97316' }} />
            Actual Expenses
          </span>
        </div>
      </div>
      ...
      <ResponsiveContainer width="100%" height={340}>
        ...
        >
          ...
          <Legend
            iconType="circle"
            iconSize={10}
            wrapperStyle={{ fontSize: '13px', paddingTop: '16px', color: '#64748b', fontWeight: 500 }}
          />
          <Line
            type="monotone"
