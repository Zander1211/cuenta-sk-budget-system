                ...
                {displayedDocuments.length > 0 ? (
                  displayedDocuments.map((doc) => (
                    <tr key={doc.id} style={{ opacity: doc.archivedAt ? 0.7 : 1 }}>
                      <td data-label="Document Name" style={{ fontWeight: 500 }}>{doc.name}</td>
                      <td data-label="Project/Event">{doc.project || '—'}</td>
                      <td data-label="Date Generated">{new Date(doc.dateGenerated).toLocaleString()}</td>
