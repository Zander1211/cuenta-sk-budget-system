...
function ProjectsPage() {
  ...
  <div style={{ marginTop: '12px' }}>
    ...
    {['SK Chairman', 'SK Treasurer'].includes(role) && (
      <div style={{ display: 'flex', gap: '8px', marginTop: '12px', alignItems: 'center' }}>
        <button type="button" className="secondary-button" disabled={uploadingId === project.id} onClick={() => triggerCamera(project)}>📷 Scan project Receipt</button>
        <button type="button" className="secondary-button" disabled={uploadingId === project.id} onClick={() => triggerUpload(project)}>{uploadingId === project.id ? 'Uploading...' : '📁 Upload project Receipt'}</button>
